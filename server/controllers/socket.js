...
websocket.on('connection', (socket) => {
  ...
  socket.on('getHelp', (activeBeacon) => {
    ...
    db
    .query(`select "socket", st_distance_sphere(geometry, ST_MakePoint(40.750486, -73.976401)) from "dynamicResponders" WHERE ST_DWithin(geometry, ST_MakePoint(${currentLocation[0]}, ${currentLocation[1]})::geography, ${radius}) AND available = TRUE ORDER BY geometry <-> 'Point(${currentLocation[0]} ${currentLocation[1]})'::geometry`)
    .then((responders) => {
