...
websocket.on('connection', (socket) => {
  ...
  socket.on('getHelp', (activeBeacon) => {
    ...
    db
    .query(`select "socket", st_distance_sphere(geometry, ST_MakePoint(${activeBeacon.loc[0]}, ${activeBeacon.loc[1]}) from "dynamicResponders" WHERE ST_DWithin(geometry, ST_MakePoint(${activeBeacon.loc[0]}, ${activeBeacon.loc[1]})::geography, ${radius}) AND available = TRUE ORDER BY geometry <-> 'Point(${activeBeacon.loc[0]} ${activeBeacon.loc[1]})'::geometry`)
    .then((responders) => {
